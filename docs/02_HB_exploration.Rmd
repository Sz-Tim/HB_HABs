...
# TODO Next, add observed lnlambdap1
sampling.df <- makeSimSamples(hydro.par,
                              "D:\\hydroOut\\WestCOMS2\\Archive\\netcdf_2019\\",
                              "..\\..\\01_FVCOM\\data\\WeStCOMS2_Mesh.shp",
                              site.xy)
...
sampling.df <- sampling.df %>% 
  mutate(phytDens=sample(fsa.df$pseudo_nitzschia_sp, n(), replace=T),
         lnPhytDensp1=log(phytDens+1),
         lambda=phytDens + c(exp(samp.mx %*% betas$value)),
         lnlambda=log(lambda),
         lnlambdap1=log(lambda+1))
  # mutate(lnlambdaSampled=lnlambda + c(samp.mx %*% betas$value),
  #        phytDens=rpois(n(), exp(lnlambdaSampled)),
  #        lnPhytDensp1=log(phytDens+1))
...
fit <- brms::brm(lnPhytDensp1 ~ temp_sc + I(temp_sc^2) + short_wave_sc + zeta_sc, 
                 data=sampling.df, family=gaussian())
pp_check(fit)
