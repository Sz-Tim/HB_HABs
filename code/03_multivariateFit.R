...
for(i in length(covariate_sets)) {
  ...
  s_p <- glue("p{covar_s}")
  mv.prior_i <- expand_grid(sp=species, 
                            covar=covar_s) %>%
    mutate(ord=glue("NcatNum{sp} | thres(3)"),
           bern=glue("Nbloom{sp}"),
           nl_b=glue("b{covar}"),
           nl_p=glue("p{covar}"))
  ...
  priors.ordP <- c(
    map(unique(mv.prior_i$ord), 
        ~prior_string("normal(0,1)", class="b", nlpar="bIntercept", resp=.x)) %>%
      do.call('c', .),
    map(1:nrow(mv.prior_i),
        ~c(prior_string("beta(0.2,1)", nlpar=mv.prior_i$nl_p[.x], resp=mv.prior_i$ord[.x], lb=0, ub=1),
           prior_string("normal(0,1)", class="b", nlpar=mv.prior_i$nl_b[.x], resp=mv.prior_i$ord[.x]),
           prior_string("normal(0,.5)", class="sd", nlpar=mv.prior_i$nl_p[.x], resp=mv.prior_i$ord[.x], lb=0),
           prior_string("normal(0,.5)", class="sd", nlpar=mv.prior_i$nl_b[.x], resp=mv.prior_i$ord[.x], lb=0),
           prior_string("double_exponential(0,0.1)", class="sds", nlpar=mv.prior_i$nl_b[.x], resp=mv.prior_i$ord[.x], lb=0))) %>%
      do.call('c', .),
    map(covar_date, ~prior_string("double_exponential(0,0.1)", class="b", nlpar=paste0("b", .x))) %>% 
      do.call('c', .)
  )
  priors.bernP <- c(
    map(unique(mv.prior_i$bern), 
        ~prior_string("normal(0,1)", class="b", nlpar="bIntercept", resp=.x)) %>%
      do.call('c', .),
    map(1:nrow(mv.prior_i),
        ~c(prior_string("beta(0.2,1)", nlpar=mv.prior_i$nl_p[.x], resp=mv.prior_i$bern[.x], lb=0, ub=1),
           prior_string("normal(0,1)", class="b", nlpar=mv.prior_i$nl_b[.x], resp=mv.prior_i$bern[.x]),
           prior_string("normal(0,.5)", class="sd", nlpar=mv.prior_i$nl_p[.x], resp=mv.prior_i$bern[.x], lb=0),
           prior_string("normal(0,.5)", class="sd", nlpar=mv.prior_i$nl_b[.x], resp=mv.prior_i$bern[.x], lb=0),
           prior_string("double_exponential(0,0.1)", class="sds", nlpar=mv.prior_i$nl_b[.x], resp=mv.prior_i$bern[.x], lb=0))) %>%
      do.call('c', .),
    map(covar_date, ~prior_string("double_exponential(0,0.1)", class="b", nlpar=paste0("b", .x))) %>% 
      do.call('c', .)
  )
  ...
  # TODO: Stopped here. Need to adjust covar_date in priors. Simpler to just make the intercept ~ s(date) maybe? Should be identical I think.
